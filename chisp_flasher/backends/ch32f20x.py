...
DEVICE_TYPE_D8 = 0x18
DEVICE_TYPE_COMPACT = 0x20


def _cand(chip_id: int, chip_type: int, erase_kib: int, *, option_profile: str = 'fv20x_or_compact') -> dict:
    return {
...
CH32F203_CANDIDATES = [
    _cand(0x30, DEVICE_TYPE_D8, 256),
    _cand(0x31, DEVICE_TYPE_D8, 256),
    _cand(0x33, DEVICE_TYPE_D8, 128),
    _cand(0x30, DEVICE_TYPE_COMPACT, 64),
    _cand(0x31, DEVICE_TYPE_COMPACT, 64),
    _cand(0x32, DEVICE_TYPE_COMPACT, 64),
    _cand(0x33, DEVICE_TYPE_COMPACT, 32),
]
CH32F205_CANDIDATES = [_cand(0x50, DEVICE_TYPE_D8, 128)]
CH32F207_CANDIDATES = [_cand(0x70, DEVICE_TYPE_D8, 256)]
CH32F208_CANDIDATES = [
    _cand(0x80, DEVICE_TYPE_D8, 128),
    _cand(0x81, DEVICE_TYPE_D8, 128),
]


class Backend(NativeUsbFamilyBackend):
    family_name = 'ch32f20x'
    def supported_protocol_variants(self) -> list[str]:
        return ['usb_native_plain', 'uart_framed']


    chip_uart_defaults = {
        'CH32F203': dict(CH32F203_CANDIDATES[0], identify_candidates=CH32F203_CANDIDATES),
        'CH32F205': dict(CH32F205_CANDIDATES[0], identify_candidates=CH32F205_CANDIDATES),
        'CH32F207': dict(CH32F207_CANDIDATES[0], identify_candidates=CH32F207_CANDIDATES),
        'CH32F208': dict(CH32F208_CANDIDATES[0], identify_candidates=CH32F208_CANDIDATES),
    }
    ...
    def read_config_uart_framed(self, chip_name: str, **kwargs):
        ...
        if result.get('cfg12_hex'):
            result.update(decode_config_fields('fv20x_or_compact', bytes.fromhex(result['cfg12_hex'])))
        return result

    def write_config_uart_framed(self, chip_name: str, config, **kwargs):
        result = self._uart_helper().write_config_chip(chip_name=chip_name, config=config, option_profile='fv20x_or_compact', **self._uart_detect_defaults(chip_name), **kwargs)
        if result.get('cfg12_hex'):
            result.update(decode_config_fields('fv20x_or_compact', bytes.fromhex(result['cfg12_hex'])))
        return result
